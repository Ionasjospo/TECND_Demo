...
COOLDOWN_SECONDS = 3.0       # tiempo mínimo entre envíos
CONFIRM_FRAMES   = 3         # frames seguidos con defecto para confirmar
...
def confidence(area, denom_area):
    ...
    return float(min(1.0, rel * 10.0))  # escala simple para demo
...
def run_video(video_path: str = "assets/galletitas.mp4"):
    if not os.path.isfile(video_path):
        print(f"No existe el archivo de video: {video_path}")
        return
    ...
    # ROI (estación de control)
    use_roi = True
    ROI = (220, 80, 520, 380)  # x, y, w, h (ajústalo a tu video)
